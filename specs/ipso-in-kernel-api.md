---
name: ipso in-kernel API
overview: Implement the ipso in-kernel Python library — the public API (execute_cell, subtest, register_teardown), the runner-facing API (_runner submodule), and shared mutable state (_state module).
todos: []
isProject: false
---
...
The `ipso` package runs inside a Jupyter kernel. It has two consumers:

- **Test authors** (and fixture source code): use the public API — `execute_cell()`, `subtest()`, and `register_teardown()`.
- **The pytest runner** (the `pytest-ipso` package, out of scope here): communicates with the kernel over ZMQ by sending `execute_request` messages. It calls `ipso._runner` functions to inject data into the kernel and reads results back out via `ipso._runner.get_test_results()`.

Data flows **both ways**:

- **Into the kernel**: the runner calls `ipso._runner.load_cell(source)` to inject the patched cell source before the test source runs.
- **Out of the kernel**: the runner calls `ipso._runner.get_test_results()` after the test source finishes to retrieve the accumulated subtest results as a JSON string.
...
The runner retrieves results via `ipso._runner.get_test_results()`, which returns a JSON string. Deserialized, it is a list of dicts, one per subtest:
...
If `test_results` is empty after the test source finishes (no `subtest()` calls were made), the **runner** (not this package) constructs a single implicit result using the test name. The in-kernel `ipso` package only accumulates results via `subtest()` — implicit subtest handling is runner-side.
